...
def create_lambda_schedule(n_T: int, lambda_start: float = 0.001, lambda_end: float = 0.999, power: float = 2.0, device: Optional[torch.device] = None) -> torch.Tensor:
    ...
    t = torch.from_numpy(sqrt_lambda) 
...
class FeatureBridge(nn.Module):
    ...
    def predict_x_future(self, x_t, cond, x_hist, poi, t_idx, context_mask):
        ...
        sigma_t = self.diff.gamma * sqrt_lambda_t 
        
        x_future_hat = (x_t - lambda_t * x_hist - sigma_t * eps_pred) / (1.0 - lambda_t) 
        return x_future_hat

class DiffRes(nn.Module):
    def __init__(self, backbone: nn.Module, n_T: int = 15, device: str = "cpu", drop_prob: float = 0.1, gamma: float = 1.0, lambda_start: float = 0.001, lambda_end: float = 0.999, power: float = 2.0): 
        super().__init__()
        self.n_T = int(n_T)
        ...
        sqrt_lambdas_t = create_lambda_schedule(self.n_T, lambda_start=lambda_start, lambda_end=lambda_end, power=power, device=None)  
        lambdas_t = sqrt_lambdas_t ** 2  # length n_T

        zero = torch.tensor([0.0], dtype=torch.float32)
        lambdas_full = torch.cat((zero, lambdas_t.to(zero.device)), dim=0).to(self.device)    
        sqrt_lambdas_full = torch.cat((zero, sqrt_lambdas_t.to(zero.device)), dim=0).to(self.device)

        alpha_diff = lambdas_full[1:] - lambdas_full[:-1]

        alpha_full = torch.cat((zero.to(self.device), alpha_diff), dim=0) 


        self.register_buffer("lambdas", lambdas_full)      
        self.register_buffer("sqrt_lambdas", sqrt_lambdas_full) 
        self.register_buffer("alpha", alpha_full)
        self.to(self.device)

    def _predict_x0_from_backbone(self, x_t, cond, x_hist, poi, t_idx, context_mask): 
        if hasattr(self.backbone, "predict_x_future"): 
            return self.backbone.predict_x_future(x_t, cond, x_hist, poi, t_idx, context_mask) 
        t_frac = (t_idx.float() / max(1, self.n_T)).to(x_t.device)
        return self.backbone(x_t, cond, x_hist, poi, t_frac, context_mask) 

    def q_sample(self, x_future: torch.Tensor, x_hist: torch.Tensor, t: torch.LongTensor, noise: Optional[torch.Tensor] = None) -> torch.Tensor: 
        B = x_future.shape[0] 
        if noise is None:
            noise = torch.randn_like(x_future) 
        lambda_t = self.lambdas[t].view(B, *([1] * (x_future.dim() - 1))).to(x_future.device) 
        sqrt_lambda_t = self.sqrt_lambdas[t].view_as(lambda_t).to(x_future.device) 
        sigma_t = self.gamma * sqrt_lambda_t 
        x_t = lambda_t * x_hist + (1 - lambda_t) * x_future + sigma_t * noise 
        return x_t
    ...
    def forward(self, x_future: torch.Tensor, cond, x_hist: torch.Tensor, poi) -> torch.Tensor:
        B = x_future.shape[0]
        t = torch.randint(1, self.n_T + 1, (B,), device=self.device).long()
        x_t = self.q_sample(x_future, x_hist, t)
        context_mask = torch.bernoulli(torch.zeros(B, device=self.device) + self.drop_prob).to(self.device)
        pred_x_future = self._predict_x0_from_backbone(x_t, cond, x_hist, poi, t, context_mask)
        loss = F.mse_loss(pred_x_future, x_future)
        return loss

    @torch.no_grad()
    def sample(self, n_sample: int, size: tuple, cond: torch.Tensor, x_hist: torch.Tensor, poi: torch.Tensor, guide_w: float = 0.0, device: Optional[torch.device] = None) -> torch.Tensor: 
        device = device if device is not None else x_hist.device 
        cond_dup = cond.repeat(2, 1, 1, 1).to(device)
        x_hist_dup = x_hist.repeat(2, 1, 1, 1).to(device) 
        poi_dup = poi.repeat(2, 1, 1, 1).to(device)
        context_mask = torch.zeros(n_sample, device=device).repeat(2)
        context_mask[n_sample:] = 1.0

        lambda_T = self.lambdas[self.n_T].to(device) 
        sigma_T = self.gamma * lambda_T.sqrt() 
        x_t = x_hist[:n_sample].to(device) + torch.randn(n_sample, *size, device=device) * sigma_T 
        ...
        for t in range(self.n_T, 0, -1):
            ...
            pred_all = self._predict_x0_from_backbone(x_in, cond_dup, x_hist_dup, poi_dup, t_idx, context_mask) 
            pred_cond = pred_all[:n_sample]
            pred_uncond = pred_all[n_sample:]
            pred_x_future = (1 + guide_w) * pred_cond - guide_w * pred_uncond if guide_w != 0.0 else pred_cond 
            x_hist0 = x_hist[:n_sample].to(device) 
            if t > 1:
                lambda_prev = self.lambdas[t-1].view(1,1,1,1).to(device) 
                alpha_t = (self.lambdas[t] - self.lambdas[t-1]).view(1,1,1,1).to(device) 
                mean_prev = x_hist0 + (1 - lambda_prev) * (pred_x_future - x_hist0)

                z = torch.randn_like(x_t)
                lambda_prev_for_sigma = self.lambdas[t-1].to(device) 
                lambda_prev_minus_1_for_sigma = self.lambdas[t-2].to(device) if t > 2 else torch.tensor(0.0, device=device)
                sigma_prev = self.gamma * torch.sqrt(lambda_prev_for_sigma * (1 - lambda_prev_minus_1_for_sigma)).view(1,1,1,1) 
                x_t = mean_prev + sigma_prev * z 
            else:
                x_t = pred_x_future 
